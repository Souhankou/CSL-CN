...
namespace HanJie.CSLCN.Services
{
    public class MenuService : BaseService<MenuDto, Menu>
    {
        ...
        public async Task<List<Menu>> GetAllMainMenusAsync()
        {
            ...
            results = await base.CSLDbContext.Menus.Where(m => m.MenuType == MenuTypeEnum.MainMenu).AsNoTracking().ToListAsync();
            return results;
        }
        ...
        public async Task<List<Menu>> GetAllFirstChildsAsync()
        {
            ...
            results = await base.CSLDbContext.Menus.Where(m => m.MenuType == MenuTypeEnum.FirstChild).AsNoTracking().ToListAsync();
            return results;
        }
    }
}
